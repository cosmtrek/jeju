...
| `--output final` keeps command output limited to the agent's final answer, so it does not print the run id and report path. | The evidence trail requires a separate lookup after downstream polish, rewrite, or translation runs. | CLI. This is the intentional final-output contract; use `jeju view` or `jeju inspect` to retrieve run evidence. |
